...
{
/**
 * @brief Configuration object containing Snowplow collector settings used to
...
public:
     ...
     *
     * @param collector_url Full URL of the Snowplow collector including the
     * protocol (or defaults to HTTPS if protocol not present).
     * @param method HTTP method to use when sending events to collector – GET
     * or POST.
     * @param curl_cookie_file Path to a file where to store cookies in case the
     * CURL HTTP client is used – only relevant under Linux (CURL is not used
     * under Windows and macOS)
     */
    NetworkConfiguration(const string& collector_url,
                         Method method = POST,
                         const string& curl_cookie_file = "");
    ...
    /**
     * @return string Path to a file where to store cookies in case the CURL
     * HTTP client is used – only relevant under Linux (CURL is not used under
     * Windows and macOS)
     */
     ...
     *
     * @param http_client Unique pointer to a custom HTTP client to send GET and
     * POST requests with.
     */
    void set_http_client(unique_ptr<HttpClient> http_client)
    {
...
private:
    /**
     * @brief Retrieve and take ownership of the configured HTTP client. Can
     * only be called once – by the Emitter.
     *
     * @return unique_ptr<HttpClient> Unique pointer to the HTTP client.
     */
    unique_ptr<HttpClient> move_http_client()
    {
        return m_http_client ? move(m_http_client) : nullptr;
    }
