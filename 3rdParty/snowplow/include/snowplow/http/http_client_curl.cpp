...
HttpClientCurl::~HttpClientCurl() { curl_global_cleanup(); }

const string HttpClientCurl::TRACKER_AGENT =
    string("Snowplow C++ Tracker (Unix)");

static size_t
write_data(void* data, size_t byte_size, size_t n_bytes, std::string* body)
{
    return byte_size * n_bytes;
}

HttpRequestResult HttpClientCurl::http_request(const RequestMethod method,
                                               CrackedUrl url,
                                               ...
                                               bool oversize)
{
    ...
    headers =
        curl_slist_append(headers, ("User-Agent: " + TRACKER_AGENT).c_str());
    ...
    else
    {
        ...
        headers = curl_slist_append(
            headers, ("Content-Type: " + SNOWPLOW_POST_CONTENT_TYPE).c_str());
    }
