...
Tracker::Tracker(const TrackerConfiguration& tracker_config,
                 shared_ptr<Emitter> emitter,
                 shared_ptr<Subject> subject,
                 shared_ptr<ClientSession> client_session)
    : Tracker(move(emitter),
              move(subject),
              move(client_session),
...
Tracker::Tracker(shared_ptr<Emitter> emitter,
                 shared_ptr<Subject> subject,
                 shared_ptr<ClientSession> client_session,
...
Tracker::~Tracker() { this->stop(); }

// --- Controls

void Tracker::start() { this->m_emitter->start(); }

void Tracker::stop() { this->m_emitter->stop(); }

void Tracker::flush() { this->m_emitter->flush(); }
...
string Tracker::track(const Event& event)
{
    ...
    {
        context.push_back(
            this->m_client_session->update_and_get_session_context(
                payload.get_event_id(), payload.get_timestamp()));
    }
    ...
    if (context.size() > 0)
    {
        ...
        SelfDescribingJson context_json(SNOWPLOW_SCHEMA_CONTEXTS,
                                        context_data_array);
        payload.add_json(context_json.get(),
                         m_use_base64,
                         SNOWPLOW_CONTEXT_ENCODED,
                         SNOWPLOW_CONTEXT);
    }
