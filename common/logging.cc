...
void initLogger()
{
    ...
    if (is_save_log_file)
    {
        ...
        rotateSink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(logfile,
                                                                            1024 * 1024 * 10,
                                                                            3);
        rotateSink->set_level(getLogLevel());
        sinkList.push_back(rotateSink);
    }
    m_logger = std::make_shared<spdlog::logger>("multi_sink", begin(sinkList), end(sinkList));
